...
function browserResize() {
  ...
  if (k42 == true) {
    if (document.getElementById("adngin-main_leaderboard-0")) {
      if (Number(PAN_getStyleValue(document.getElementById("main"), "height").replace("px", "")) > 2200) {
...
function PAN_close_all_nav() {
  PAN_close_all_topnav();
  close_menu();
}
...
(function () {
  ...
  for (i = 0; i < x.length; i++) {
    ...
    b = x[i].pathname;
    if ((x[i].parentNode.tagName == "LI" || (a != "/" && x[i].parentNode.className.indexOf("PAN-bar") > -1) && a.substr(0, a.indexOf("/", 1)) == b.substr(0, b.indexOf("/", 1)))) {
      x[i].className += " active";
    }
  }
  if (window.addEventListener) {
    document.getElementById("main").addEventListener("click", PAN_close_all_nav, true);
    if (m) m.addEventListener("click", PAN_close_all_topnav, true);
    document.getElementById("right").addEventListener("click", PAN_close_all_nav, true);
    document.getElementById("main").addEventListener("wheel", PAN_close_all_nav, true);
    document.getElementById("main").addEventListener("touchstart", PAN_close_all_nav, true);
  } else if (window.attachEvent) {
    document.getElementById("main").attachEvent("onclick", PAN_close_all_nav);
    if (m) m.attachEvent("onclick", PAN_close_all_topnav);
    document.getElementById("right").attachEvent("onclick", PAN_close_all_nav);
  }
...
function searchfield_focus(obj) {
  obj.style.color = "";
  obj.style.fontStyle = "";
  if (obj.value == "Search Piparaacademy.com") { obj.value = ""; }
}
...
function clickFBLike() {
  ...
  document.getElementById("popupDIV").innerHTML = "<iframe src='/fblike.asp?r=" + Math.random() + "' frameborder='no' style='height:200px;width:250px;'></iframe><br><button onclick='hideFBLike()' class='PAN-btn PAN-black'>Close</button>";
}
function hideFBLike() {
...
/* PANcodecolor ver 1.32 by Piparaacademy.com */
(
  function PANCodeColor() {
    ...
    for (j = 0; j < k; j++) {
      ...
      for (i = 0; i < l; i++) {
        ...
        //        codemirrorize(x[i]);
        //      } else {
        x[i].innerHTML = PANCodeColorize(x[i].innerHTML + " ", modes[j]);
        //      }
      }
    }
    /*
    ...
    */
    function PANCodeColorize(x, lang) {
      ...
      function htmlMode(txt) {
        ...
        php = new extract(rest, "&lt;\\?php", "?&gt;", phpMode, "PANPHPPOS");
        rest = php.rest;
        comment = new extract(rest, "&lt;!--", "--&gt;", commentMode, "PANHTMLCOMMENTPOS");
        rest = comment.rest;
        ...
        for (i = 0; i < comment.arr.length; i++) {
          rest = rest.replace("PANHTMLCOMMENTPOS", comment.arr[i]);
        }
        for (i = 0; i < php.arr.length; i++) {
          rest = rest.replace("PANPHPPOS", php.arr[i]);
        }
        return rest;
      }
      ...
      function cssMode(txt) {
        ...
        comment = new extract(rest, /\/\*/, "*/", commentMode, "PANCSSCOMMENTPOS");
        rest = comment.rest;
        while (rest.search("{") > -1) {
          s = rest.search("{");
        ...
        rest = rest.replace(/}/g, "<span class='cssdelimitercolor' style=color:" + cssdelimitercolor + ">}</span>");
        for (i = 0; i < comment.arr.length; i++) {
          rest = rest.replace("PANCSSCOMMENTPOS", comment.arr[i]);
        }
      ...
      function jsMode(txt) {
        ...
        for (i = 0; i < esc.length; i++) {
          rest = rest.replace("PANJSESCAPE", esc[i]);
        }
