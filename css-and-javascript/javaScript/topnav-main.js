...
// ------------------------------------search function opening tags------------------------------------------------  
  ...
  function click_learntocode_search_btn() {
      ...
      for (i = 0; i < x.length; i++) {
          if (x[i].className.indexOf("search_active") > -1) {
              n = x[i].href;
              if (n.indexOf("search_entire_PANschools") > -1) {
                  cc = 1;
              }
              break;
          }
  ...
  function find_search_results(inp) {
      ...
      if (resultIndexes.length == 0) {
          classAtt = "";
          if (html == "") classAtt = " search_active";
          // html += "<a class='search_item" + classAtt + "' onclick='click_search_PANschools_link(event)' href='search_entire_PANschools'>Search PANSchools</a>";
      }
      a.innerHTML = html;
  }
  function click_search_PANschools_link(event) {
      event.preventDefault();
      submit_search_form();
  }
