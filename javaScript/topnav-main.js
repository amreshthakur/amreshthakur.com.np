    ...
    if (currentUnits.length > 0) {
        currentUnits.forEach((unit, index) => {
            ...
            unitCard.innerHTML = `
                        ...
                        <a href="${unitLink}" class="pipara-academy-course-unit-link" target="_blank">
            ...
            unitCard.addEventListener('click', function(e) {
                // Only open details if not clicking on the link
                if (!e.target.closest('.pipara-academy-course-unit-link')) {
                    showUnitDetails(unit, unitNumber);
                }
            });
            
            // Add direct link functionality
            const linkElement = unitCard.querySelector('.pipara-academy-course-unit-link');
        ...
        for (let i = 1; i <= 5; i++) {
            ...
            // Create a unit object for the fallback
            const unit = {
                title: unitTitle,
